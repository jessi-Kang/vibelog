...
- **독자는 이 프로젝트를 처음 보는 사람이다.** 커밋 메시지의 개발자 용어를
  그대로 옮기지 말고 독자의 말로 번역한다. 파일명·함수명·라이브러리명은
  꼭 필요한 한두 개만 쓰고, 쓸 때는 그게 뭘 하는 물건인지 반 문장으로 푼다.
- 숫자를 명사에 붙여 줄인 표현을 쓰지 않는다 — "5화면"이 아니라 "화면
  다섯 개", "3종"이 아니라 "세 가지"처럼 소리 내어 읽어 자연스러운 말로.
