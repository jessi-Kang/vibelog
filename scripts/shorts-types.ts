/**
 * **이 파일은 브라우저 번들에 들어간다.** video/src/*가 여기서 타입과 작은
 * 순수 함수를 가져오고, Remotion이 그걸 webpack으로 묶는다 — `node:fs` 같은
 * 노드 전용 모듈을 여기 import하면 렌더가 번들 단계에서 죽는다
 * (UnhandledSchemeError). 타입체크로는 안 잡힌다.
 *
 * 파일을 읽거나 경로를 만지는 검사기는 scripts/ 밑 별도 파일에 둔다
 * (scripts/check-screens.ts). 여기에는 타입과 순수 함수만 둔다.
 */
/**
...
export interface ShortsLine {
  ...
  diagram?: DiagramSpec;
  /**
   * 이 문장의 빈 자리를 채울 **라벨 없는 아이콘**. 화면(screen·find)도
   * 그림(diagram)도 못 고른 문장이 배경과 자막만으로 나가던 자리다.
   *
   * 다이어그램과 역할이 다르다 — 다이어그램은 라벨을 달아 원리를 설명하고,
   * 모티프는 라벨 없이 문장의 **모양**만 보여 준다. 반복·지켜봄·쌓임·되돌아감
   * 처럼 말로 이미 다 한 이야기에 그림을 하나 얹는 자리다.
   *
   * **마지막 수단이다.** 보여줄 실제 화면이 있으면 화면이 이기고, 설명할
   * 원리가 있으면 다이어그램이 이긴다. 셋 다 없을 때만 고른다.
   */
  motif?: MotifName;
}
...
export const MOTIFS = [
  ...
  "watch", // 지켜보기 · 확인 — 끝까지 도는지 본다
  "gate", // 관문 · 검토 — 사람이 본 뒤에만 지나간다
  "stack", // 쌓임 · 모으기 — 하나씩 늘어난다
  "refine", // 다듬기 · 계속 — 들쭉날쭉하던 것이 고르게
  "merge", // 둘이 하나로 — 갈라져 있던 것을 합쳤다
  "fanin", // 여럿이 한 번으로 — 많이 와도 한 번만 한다
  "limit", // 한도에 걸림 — 천장에 닿았다
  "tile", // 이어 붙이기 — 같은 토막을 필요한 만큼
  "cut", // 짧은 쪽에 맞춰 잘림
  "missing", // 있어야 할 게 없음
  "oneline", // 한 줄 때문에 전체가
  "fallback", // 옛 값으로 되돌아감
...
export interface DemoSegment {
   ...
   */
  focusY?: number;
  /** 그림 한 장짜리 화면(링크 미리보기 카드 등)의 가로세로비 — 있으면 렌더가
   *  폰 프레임 대신 그 비율의 카드로 보여 준다. 앱 화면이 아닌 것을 폰에
   *  억지로 맞추면 늘어나고 잘린다 */
  aspect?: number;
}
