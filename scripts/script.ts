...
- **두 명사를 붙여 새 말을 만들지 않는다.** 관계를 지우면 듣는 사람이 뜻을
  세우지 못한다 — "미리보기 도장"은 미리보기가 도장인지, 도장을 미리 보는
  것인지, 미리보기에 찍힌 도장인지 알 수 없다. 원문(데브로그)은 "미리보기
  그림에 도장을 찍었는데"였다: **조사를 살려 관계를 밝힌다** ("미리보기
  그림의 도장", "공유 카드에 찍은 도장").
...
async function siteScreens(url: string): Promise<SiteScreen[]> {
  ...
  //
  // 이게 없으면 "미리보기 그림의 도장이 얼룩으로 나왔습니다" 같은 문장이 가리킬 곳이
    ...
    try {
      ...
      if (u.origin === new URL(url).origin) {
        ...
        screens.push({
          ...
          // 무엇이 든 그림인지는 이름에 적는다 (alt) — 대본이 어떤 문장에
          // 쓸지 고르는 근거다
          label: `링크 미리보기 카드, 그림 한 장 (공유할 때 뜨는 것${
            alt ? `: ${alt.slice(0, 40)}` : ""
          })`,
          // **보이는 글자는 없다.** 이 화면은 PNG 한 장이라 글자가 DOM에
          // 없다. alt를 여기 넣었더니 대본이 그게 화면에 보이는 줄 알고
          // find로 골랐고, 녹화기는 그 글자를 카드에서 못 찾아 같은 말이
          // 있는 홈으로 끌려갔다 — 결국 홈이 두 번 나왔다.
          sees: [],
        });
      }
    } catch {
 ...
 * 빈 화면이 든 영상보다 낫다.
 *
 * hook·end만 제 카드가 있으니 빈 화면이 아니다.
 *
 * fail은 예외에서 **뺐다.** 삽질 카드(failCard)는 문제·해결을 적은 제목 카드일
 * 뿐이어서, 그 문장이 말하는 것을 보여주지는 않는다. 이 예외 때문에 "미리보기
 * 도장이 얼룩처럼 번졌습니다" 같은 문장이 화면도 그림도 없이 통과했고, 정작
 * 도장은 옆 문장(게임 설명)이 가리킨 화면에 딸려 나왔다 (Jessi: "스탬프 말할
 * 땐 안 나오고 진짜 가짜 게임이라는 설명할 때 나오고 있어"). 삽질 문장도 둘 중
 * 하나를 고른다 — 그것을 화면에서 가리키거나(find), 원인을 그린다(diagram).
    ...
    ...(screens.length >= 2
      ? [
          [
            ...
            "  **그림 한 장이라 가리킬 글자가 없다 — find를 비우고 shot은",
            '  "whole"로 둔다.** 카드 안의 무엇을 말하는 문장이어도 그렇다.',
            "- **같은 화면을 두 문장 넘게 쓰지 마라.** 같은 페이지가 비율만",
            "  달리 되풀이되면 볼 것이 없는 영상이 된다. 쓸 화면이 모자라면",
            "  그 문장에 diagram을 붙이는 게 낫다.",
            "- **삽질(fail) 문장도 예외가 아니다.** 삽질 카드는 문제·해결을 적은",
            "  제목 카드일 뿐이어서 그 문장이 말하는 것을 보여주지 못한다.",
            "  증상이 화면에 보이는 것이면 find로 가리키고(글자가 없는 도장·그림도",
            "  부르는 말을 적으면 녹화기가 찾는다), 눈에 안 보이는 원인이면",
            "  diagram을 붙인다. 도장 이야기를 하는데 도장이 한 번도 안 나오면",
            "  영상이 실패한 것이다.",
