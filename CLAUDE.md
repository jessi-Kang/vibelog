...
- **화면도 그림도 안 고른 문장은 장애다** (Jessi: "대본이 화면도 그림도 안 고르면 장애야. 그럼 영상이 만들어지면 안 되지"). `blankScenes`가 잡아 한 번 다시 쓰게 하고, 그래도 비면 예외를 던져 **영상을 만들지 않는다** — 쇼츠가 없는 날이 빈 화면이 든 영상보다 낫다. 면제는 `hook`·`end`뿐이다. `fail`도 면제에서 뺐다: 삽질 카드는 제목 카드일 뿐이어서, 면제해 두면 "도장 이야기를 하는데 도장이 한 컷도 없는" 편이 구조적으로 보장된다 (실제로 도장은 옆 문장이 가리킨 화면에 딸려 나왔다).
- **가리킨 것의 자리를 렌더에 넘긴다 (`focusY`).** `focus` 샷은 폰 화면을 밴드로 크롭하므로 기준을 가운데로 박아 두면 위·아래 요소가 잘린다 (Jessi: "제대로 표시했는데 영상에서 보여줄 때 짤렸잖아"). 녹화가 요소의 세로 위치(0~1)를 구간에 적고, 렌더가 그 지점을 크롭 기준(`objectPosition`)으로 쓴다. 녹화에서 화면을 미는 방식은 밀린 만큼이 크롭 경계에 걸려 되레 잘렸다.
- **구성은 내용이 정한다 — 개수·순번·로테이션으로 정하지 않는다** (Jessi, 여러 번 지적). 샷 로테이션·다이어그램 개수 상한·stat 개수 상한·이웃 회피 규칙을 전부 걷어냈다. 무엇을 화면으로 보여주고 무엇을 그림으로 설명할지는 그 문장이 무슨 말을 하는지로만 고른다. 남은 상한(문장 수·길이·자막 두 줄·강조 절반 이하)은 읽기·규격 제약이다. 표와 이유는 docs/03-shorts-spec.md "구성은 내용이 정한다".
