...
- 데브로그 형식: 뭘 했다 / 왜 / 삽질 포인트 / 다음 할 것 / 스크린샷.
...
- 쇼츠의 데모 화면 소재는 항상 실제 배포 사이트 녹화 (텍스트→비디오로 데모를 대체하지 않는다 — 가짜 화면이 된다). 단, 생성 그래픽(일러스트)은 +알파로 적극 활용한다: `scripts/art.ts` + `GEMINI_API_KEY`, 상세는 docs/03.
