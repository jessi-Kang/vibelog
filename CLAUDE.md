...
- **"오늘" 라벨 지표는 KST 자정 기준 리셋.** 파이프라인이 projects.json에 `countsDate`(집계 기준 날짜)를 남기고, 화면(`components/home-facts.tsx` 등)이 방문 시점의 오늘과 클라이언트에서 비교한다 — 정적 빌드 값은 자정이 지나면 어제 것이라서. "마지막 활동"의 오늘/어제도 같은 방식(`LastActive`).
