...
- **푸시했으면 배포 상태를 확인한다** (Jessi 지시 — "배포하면 배포 상태를 확인해야지"). 커밋·푸시는 끝이 아니다. Vercel 프로덕션 배포가 READY가 될 때까지 보고, ERROR면 빌드 로그를 읽어 고친다. 실제로 두 커밋이 OG 폰트 fetch 타임아웃으로 연속 ERROR인 채 라이브에 못 나가 있었는데, 푸시 결과만 보고 "적용됐다"고 말한 탓에 몇 시간을 모르고 지났다.
- **안내에는 항상 링크를 넣는다** (Jessi 지시 — "내가 항상 링크를 포함해서 말해달라 했지"). Jessi가 할 일을 알려 줄 때는 눌러서 바로 갈 주소를 같이 준다: 설정 화면, 워크플로 실행 화면, 배포 목록, 발급 페이지. "어디어디 들어가서"로 끝내지 않는다.
