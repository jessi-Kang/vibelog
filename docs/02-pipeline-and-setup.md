...
가드는 **날짜 버킷이 아니라 마지막 발행으로부터의 시간 간격**으로 본다
(12시간). 날짜로 보던 때 그 틈으로 한 회차가 빠져나갔다 — 2026-09-15 실측:
03:00 예정 백업이 3시간 6분 밀려 06:06에 떴고, 경계가 06:00이라 "9/15 밤"
회차로 계산됐다. 일곱 시간 전에 끝난 9/14 밤 발행은 날짜가 달라 가드에 안
걸렸고, 새벽 여섯 시에 커밋 하나짜리 9/15 글과 영상이 그대로 나갔다. 밤
회차는 24시간 간격이므로, 12시간 안에 발행 기록이 있으면 경계가 어느 쪽으로
갈렸든 같은 밤의 중복이다.
...
밤 파이프라인의 돈은 9할이 Opus 5 출력이고, 그 출력의 3/4이 보이지 않는
thinking이다 (삽화 실측: 보이는 SVG 2,900토큰 vs 출력 11,000–32,000). 그래서
모든 Opus 호출이 `scripts/usage.ts`로 토큰을 남기고, 홈 "지난 실행"에
`토큰 in N · out M · cache K`로 찍힌다. 손잡이를 돌리면 다음 밤 이 줄로 판단한다.

| 항목 (글 1편) | 편당 | 근거 |
|---|---|---|
| 글 본문 (Opus, ko+en) | $0.2–0.4 어림 | 입력 3–6천 + thinking |
| 삽화 (Opus) | $0.32–0.90 실측 | 4편 중 4편이 재작성 |
| 쇼츠 대본 (Opus) | $0.3–0.5 어림 | 시스템 프롬프트 3,300 + 글 전문 |
| 내레이션 ko+en (ElevenLabs multilingual_v2) | $0.07 | 편당 약 650자 |
| 렌더·Actions·Blob | 거의 0 | 공개 레포 |

적용한 손잡이 (순서대로, 앞이 무료):
1. 시스템 프롬프트 `cache_control` — 글·대본·삽화 프롬프트는 밤마다 같다.
2. 삽화 재작성 다이어트 — 탈락 사유가 전부 "본문에 없는 숫자"라 섹션마다
   숫자 목록을 미리 주고, 재작성엔 걸린 항목만 되돌린다.
3. 활동 없는 밤의 헛실행 가드 — `run.json`의 at이 12시간 안이면 백업 슬롯 종료.
4. effort "처음 medium, 걸리면 high" — 삽화·대본만. 글 본문은 기본값.
남은 것: Batch API(밤 전체 반값, 구조 변경이라 4의 실측 뒤), 영어를 Opus가 두 번
쓰지 않기(Sonnet 번역), Sonnet 5로 내리기(마지막).
