...
## Assets
- 로고 없음. 워드마크는 텍스트(`vibelog` **JetBrains Mono 700** — 코드 느낌, Jessi 지시. 쇼츠 타이포의 Noto Sans KR 900은 별개). 파비콘·앱 아이콘은 **`v_` 레터마크**(JetBrains Mono Bold, 자연 모노 간격 그대로 — 커닝 등 변형 금지, Jessi 확정. app/icon.svg). 크기는 좌우 여백 15px(64 기준) — 삼성 One UI 스퀘클에 얹어 고른 값. OG 카드는 모노 워드마크.
